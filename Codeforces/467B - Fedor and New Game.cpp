...
void solve() {
    ...
    ll can_become_friends_with = 0;
    rep(i, 0, m)
    {
        ll differ_bits = x[i] ^ x[m];
        ll count_differ_bits = __builtin_popcount(differ_bits);
        if (count_differ_bits <= k)
        {
            can_become_friends_with += 1;
        }
    }
    cout << can_become_friends_with;
}
