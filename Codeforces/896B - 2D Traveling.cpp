...
void solve() {
    //code goes here
	  ...
	  if(k<=1){
	    ...
	    for(int i=0;i<k;i++){
	      ll temp=abs(v[i].first-v[a-1].first)+abs(v[i].second-v[a-1].second);
	      mini=min(mini,temp);
	    }
	    for(int i=0;i<k;i++){
	      ll temp=abs(v[i].first-v[b-1].first)+abs(v[i].second-v[b-1].second);
	      minii=min(minii,temp);
	    }
	    ans=min(mini+minii,abs(v[a-1].first-v[b-1].first)+abs(v[a-1].second-v[b-1].second));
	  }
